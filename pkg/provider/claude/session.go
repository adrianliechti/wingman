...
// sessionStore caches CLI session_ids keyed by a canonical hash of the
// conversation prefix, so the next Complete() call can re-attach via
// `--resume <session_id> --fork-session` instead of replaying history through
// the system prompt.
...
func (s *sessionStore) put(key, sessionID string) {
	if s == nil || key == "" || sessionID == "" {
		return
	}
...
// keyFor computes a stable hash over the conversation. We hash only the
// caller-controlled, round-trip-stable parts: system + user message contents
// (text, attached files, and tool_result blocks the caller supplied to
// answer prior tool_use). Assistant messages are deliberately ignored —
// callers may strip thinking signatures or normalise JSON formatting when
// echoing them back, which would silently break the cache.
